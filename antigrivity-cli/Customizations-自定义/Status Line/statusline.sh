...
# ─── Output ──────────────────────────────────────────────────────────────────
LINE1="${S}${M}${V}"
LINE2="${CTX}${DOT}${ART_FMT}${DOT}${SUB_FMT}${DOT}${BG_FMT}${DOT}${SB}"    # stats
LINE3="${GEMINI_QUOTA}${DOT}${TP_QUOTA}"                                     # quota

# ─── Visible-length estimation (strip ANSI, count display characters) ────────
__vlen() { echo -e "$1" | awk '{ gsub(/\[[0-9;]*[a-zA-Z]/, ""); print length; }'; }
L1_V=$(__vlen "$LINE1")
L2_V=$(__vlen "$LINE2")
L3_V=$(__vlen "$LINE3")
...
# All three segments + separators fit → one-line with pipe separators
ONE_LINE_OK=$(( L1_V + PIPE_V + L2_V + PIPE_V + L3_V ))
if [ "$COLS" -ge "$ONE_LINE_OK" ]; then
  echo -e "${LINE1}${FG_GRAY}${PIPE}${R}${LINE2}${FG_GRAY}${PIPE}${R}${LINE3}"

# Multi-line: quota on second line (box-drawing for ≥80, compact below)
elif [ "$COLS" -ge 80 ]; then
  echo -e "${FG_GRAY}╭─${R} ${LINE1}"
  echo -e "${FG_GRAY}├─${R} ${LINE3}"
  echo -e "${FG_GRAY}╰─${R} ${LINE2}"

# Narrow terminal (<80) — compact three-line, minimal chrome
else
  echo -e "${S}${M}"
