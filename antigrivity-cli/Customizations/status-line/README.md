...
如果你选择使用优化版 `statusline_with_quota.sh`，可以直接下载本仓库提供的版本（以下为下载命令，执行即可）：

```bash
curl -L https://raw.githubusercontent.com/warriors1989/antigravity-tutorial/refs/heads/master/antigrivity-cli/Customizations-%E8%87%AA%E5%AE%9A%E4%B9%89/Status%20Line/statusline_with_quota.sh -o ~/.gemini/antigravity-cli/statusline.sh
```

然后按下面的安装步骤操作，只需将**第一步**的下载命令替换为上面的命令即可。
...
```bash
curl -L https://raw.githubusercontent.com/google-antigravity/antigravity-cli/0e6f7708d525dfea6980926307134ff6ec8bc540/examples/statusline/statusline.sh -o ~/.gemini/antigravity-cli/statusline.sh
```
